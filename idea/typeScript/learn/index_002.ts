...
//10.数组去重
;() => {
  function unique1(arr: Array<number>) {
    return Array.from(new Set(arr))
  }
  function unique2(arr: Array<number>) {
    return arr.filter((item, index) => arr.indexOf(item) === index)
  }
  function unique3(arr: Array<number>) {
...
;() => {
  const nativePromiseWithExtractedComponents = () => {
    let resolver: (value: number | string | PromiseLike<string>) => void
